...
public class Customer {
    private Mogram giveFeedback() throws Exception {
        ...
        Task order = task("order", sig("makeOrder", OrderService.class), context(
                ...
                val("order/dateTime", "12-02-2021 17:15:00"),
                val("order/quantity", 3),
                val("order/price"),
                val("recipe", mocha)));

        Task payment = task("payment", sig("payWithBlik", PaymentService.class), context(
                val("payment/blikCode", "165789"),
                val("payment/dateTime", "12-02-2021 17:15:20"),
                val("payment/price"),
                val("payment/status")));
        ...
        Task feedback = task("feedback", sig("giveFeedback", FeedbackService.class), context(
                val("feedback/grade", 5),
                val("feedback/description", "Yummy!"),
                val("feedback/status")));
